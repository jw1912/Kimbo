...
fn main() {
    ...
    let position = EnginePosition::from_fen(_POSITIONS[num]);
    display_board::<true>(&position.board);

    let now = Instant::now();
    let tt = Arc::new(TT::new(32 * 1024 * 1024));
    let mut search: Search = Search::new(position, Arc::new(AtomicBool::new(false)), time, u8::MAX, u64::MAX, tt, 0);
    search.go::<true>();
