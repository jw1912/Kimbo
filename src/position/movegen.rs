...
impl Position {
    fn encode_move<const IS_KING: bool, const IS_CAPTURE: bool>(
        &self,
        ...
        while attacks > 0 {
            ...
            if IS_KING
                && self.is_square_attacked(
                    aidx as usize,
                    self.side_to_move,
                    self.occupied & !self.pieces[self.side_to_move][Piece::KING],
                )
            {
                attacks &= attacks - 1;
                continue;
    ...
    pub fn en_passants(&self, move_list: &mut MoveList) {
        ...
        let mut attackers = PAWN_ATTACKS[self.side_to_move ^ 1][sq as usize]
            & self.pieces[self.side_to_move][Piece::PAWN];
        while attackers > 0 {
    ...
    fn pawn_pushes_general<const SIDE: usize, const IN_CHECK: bool, const PINNED: bool>(
        ...
        match IN_CHECK {
            false => {
                ...
                dbl_pushable_pawns =
                    shift::<SIDE, 8>(shift::<SIDE, 8>(empty & DBLRANK[SIDE]) & empty) & pawns;
            }
            true => {
                pushable_pawns = shift::<SIDE, 8>(empty & free) & pawns;
                dbl_pushable_pawns =
                    shift::<SIDE, 8>(shift::<SIDE, 8>(empty & DBLRANK[SIDE] & free) & empty)
                        & pawns;
            }
        }
    ...
    fn pawn_captures_general<const SIDE: usize, const IN_CHECK: bool, const IS_PINNED: bool>(
        &self,
        move_list: &mut MoveList,
        checkers: u64,
        mut attackers: u64,
    ...
    fn piece_moves_general<
        const PIECE: usize,
        const IN_CHECK: bool,
        const IS_PINNED: bool,
        const IS_KING: bool,
        const TYPE: u8,
    >(
        &self,
        ...
        while attackers > 0 {
            ...
            attacks = match PIECE {
                ...
                Piece::QUEEN => {
                    rook_attacks(idx, self.occupied) | bishop_attacks(idx, self.occupied)
                }
    ...
    fn castles<const MOVETYPE: u8>(&self, move_list: &mut MoveList) {
        ...
        match self.side_to_move {
            Side::WHITE => {
                if self.castle_rights & CastleRights::WHITE_QS > 0
                    && self.occupied & (B1 | C1 | D1) == 0
                    && !self.is_square_attacked(3, Side::WHITE, self.occupied)
                    && !self.is_square_attacked(2, Side::WHITE, self.occupied)
                {
                    move_list.push(MoveFlags::QS_CASTLE | 2 << 6 | 4)
                }
                if self.castle_rights & CastleRights::WHITE_KS > 0
                    && self.occupied & (F1 | G1) == 0
                    && !self.is_square_attacked(5, Side::WHITE, self.occupied)
                    && !self.is_square_attacked(6, Side::WHITE, self.occupied)
                {
                    move_list.push(MoveFlags::KS_CASTLE | 6 << 6 | 4)
                }
            }
            Side::BLACK => {
                if self.castle_rights & CastleRights::BLACK_QS > 0
                    && self.occupied & (B8 | C8 | D8) == 0
                    && !self.is_square_attacked(59, Side::BLACK, self.occupied)
                    && !self.is_square_attacked(58, Side::BLACK, self.occupied)
                {
                    move_list.push(MoveFlags::QS_CASTLE | 58 << 6 | 60)
                }
                if self.castle_rights & CastleRights::BLACK_KS > 0
                    && self.occupied & (F8 | G8) == 0
                    && !self.is_square_attacked(61, Side::BLACK, self.occupied)
                    && !self.is_square_attacked(62, Side::BLACK, self.occupied)
                {
    ...
    fn pawn_moves<const IN_CHECK: bool, const MOVETYPE: u8>(
        &self,
        move_list: &mut MoveList,
        pinned: u64,
        king_idx: usize,
        ...
        if MOVETYPE != MoveType::QUIETS && self.en_passant_sq > 0 {
            self.en_passants(move_list)
        }
        match self.side_to_move {
            Side::WHITE => {
                if MOVETYPE != MoveType::CAPTURES {
                    self.pawn_pushes_general::<{ Side::WHITE }, IN_CHECK, false>(
                        move_list,
                        unpinned_attackers,
                        free,
                    );
                    self.pawn_pushes_general::<{ Side::WHITE }, IN_CHECK, true>(
                        move_list,
                        pinned_attackers,
                        free,
                    );
                ...
                if MOVETYPE != MoveType::QUIETS {
                    self.pawn_captures_general::<{ Side::WHITE }, IN_CHECK, false>(
                        move_list,
                        checkers,
                        unpinned_attackers,
                        king_idx,
                    );
                    self.pawn_captures_general::<{ Side::WHITE }, IN_CHECK, true>(
                        move_list,
                        checkers,
                        pinned_attackers,
                        king_idx,
            ...
            Side::BLACK => {
                if MOVETYPE != MoveType::CAPTURES {
                    self.pawn_pushes_general::<{ Side::BLACK }, IN_CHECK, false>(
                        move_list,
                        unpinned_attackers,
                        free,
                    );
                    self.pawn_pushes_general::<{ Side::BLACK }, IN_CHECK, true>(
                        move_list,
                        pinned_attackers,
                        free,
                    );
                ...
                if MOVETYPE != MoveType::QUIETS {
                    self.pawn_captures_general::<{ Side::BLACK }, IN_CHECK, false>(
                        move_list,
                        checkers,
                        unpinned_attackers,
                        king_idx,
                    );
                    self.pawn_captures_general::<{ Side::BLACK }, IN_CHECK, true>(
                        move_list,
                        checkers,
                        pinned_attackers,
                        king_idx,
    ...
    fn piece_moves<const PIECE: usize, const IN_CHECK: bool, const TYPE: u8>(
        &self,
        move_list: &mut MoveList,
        pinned: u64,
        king_idx: usize,
        ...
        self.piece_moves_general::<PIECE, IN_CHECK, false, false, TYPE>(
            move_list,
            king_idx,
            blockers,
            attackers & !pinned,
        );
        self.piece_moves_general::<PIECE, IN_CHECK, true, false, TYPE>(
            move_list,
            king_idx,
            blockers,
            attackers & pinned,
    ...
    fn gen_pnbrq_moves<const IN_CHECK: bool, const MOVETYPE: u8>(
        &self,
        move_list: &mut MoveList,
        pinned: u64,
        king_idx: usize,
        ...
        self.piece_moves::<{ Piece::KNIGHT }, IN_CHECK, MOVETYPE>(
            move_list, pinned, king_idx, blockers,
        );
        self.piece_moves::<{ Piece::BISHOP }, IN_CHECK, MOVETYPE>(
            move_list, pinned, king_idx, blockers,
        );
        self.piece_moves::<{ Piece::ROOK }, IN_CHECK, MOVETYPE>(
            move_list, pinned, king_idx, blockers,
        );
        self.piece_moves::<{ Piece::QUEEN }, IN_CHECK, MOVETYPE>(
            move_list, pinned, king_idx, blockers,
        );
    }

    // for if you want to generate moves but not recalculate checks and pinned pieces (i.e generate checks then quiets)
    pub fn gen_moves_staged<const MOVETYPE: u8>(
        &self,
        move_list: &mut MoveList,
        check_status: Check,
        king_idx: usize,
        ...
        self.piece_moves_general::<{ Piece::KING }, false, false, true, MOVETYPE>(
            move_list, 0, 0, king,
        );
        match check_status {
            Check::None => {
            ...
            Check::Single => {
                ...
                self.gen_pnbrq_moves::<true, MOVETYPE>(
                    move_list, pinned, king_idx, blockers, checks, free,
                );
            }
            Check::Double => {}
        }
